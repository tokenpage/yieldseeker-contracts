...
import {SellTokenNotAllowed, YieldSeekerAerodromeV2SwapAdapter} from "../../../src/adapters/AerodromeV2SwapAdapter.sol";
import {YieldSeekerSwapSellPolicy} from "../../../src/adapters/SwapSellPolicy.sol";
import {AWKErrors} from "../../../src/agentwalletkit/AWKErrors.sol";
import {AWKAerodromeV2SwapAdapter, IAerodromeV2Router, InvalidAerodromeV2RouterTarget, InvalidRouteLength, InvalidSwapRoute, InvalidSwapTokenAddress} from "../../../src/agentwalletkit/adapters/AWKAerodromeV2SwapAdapter.sol";
...
contract MockAerodromeV2Router {
...
contract AerodromeV2SwapAdapterTest is Test {
    YieldSeekerAerodromeV2SwapAdapter adapter;
    ...
    MockAerodromeV2Router router;

    function _decodeUint(bytes memory data) internal pure returns (uint256) {
        return abi.decode(abi.decode(data, (bytes)), (uint256));
    }

    function _buildRoute(address from, address to) internal pure returns (AWKAerodromeV2SwapAdapter.SwapRoute memory route) {
    ...
    function _executeSwap(address buyToken, AWKAerodromeV2SwapAdapter.SwapRoute memory route, uint256 sellAmount, uint256 minBuyAmount) internal returns (bytes memory) {
    ...
    function setUp() public {
        ...
        router = new MockAerodromeV2Router();
        adapter = new YieldSeekerAerodromeV2SwapAdapter(address(router), address(0xFACADE), address(sellPolicy));
    ...
    function test_Execute_Swap_Succeeds() public {
        ...
        AWKAerodromeV2SwapAdapter.SwapRoute memory route = _buildRoute(address(sellToken), address(baseAsset));
    ...
    function test_Execute_Swap_RevertsOnWrongTarget() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(InvalidAerodromeV2RouterTarget.selector, address(0x1234), address(router)));
        wallet.executeAdapter(address(adapter), address(0x1234), data);
    }

    function test_Execute_Swap_RevertsOnInvalidRouteLength() public {
        AWKAerodromeV2SwapAdapter.SwapRoute memory route;
        route.path = new address[](1);
        route.path[0] = address(sellToken);
        route.stables = new bool[](0);
    ...
    function test_Execute_Swap_RevertsOnStablePathMismatch() public {
        AWKAerodromeV2SwapAdapter.SwapRoute memory route;
        route.path = new address[](2);
        route.path[0] = address(sellToken);
    ...
    function test_Execute_Swap_RevertsOnRouteSellEndpointMismatch() public {
        AWKAerodromeV2SwapAdapter.SwapRoute memory route = _buildRoute(address(otherToken), address(baseAsset));
    ...
    function test_Execute_Swap_RevertsOnRouteBuyEndpointMismatch() public {
        AWKAerodromeV2SwapAdapter.SwapRoute memory route = _buildRoute(address(sellToken), address(otherToken));
        vm.expectRevert(abi.encodeWithSelector(InvalidRouteEndpoints.selector, address(sellToken), address(baseAsset)));
        _executeSwap(address(baseAsset), route, 100e18, 1e6);
    }
    ...
    function test_Execute_Swap_RevertsOnZeroPathToken() public {
        ...
        route.stables = new bool[](2);
        route.stables[0] = false;
        route.stables[1] = false;
    ...
    function test_Execute_Swap_RevertsWhenSellingBaseAsset() public {
        ...
        AWKAerodromeV2SwapAdapter.SwapRoute memory route = _buildRoute(address(baseAsset), address(otherToken));
