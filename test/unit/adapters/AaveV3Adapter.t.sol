...
contract AaveV3AdapterTest is Test {
    ...
    function test_Execute_Deposit_Succeeds() public {
        ...
        bytes memory result = wallet.executeAdapter(address(adapter), address(aToken), abi.encodeWithSelector(adapter.deposit.selector, amount));
        uint256 shares = _decodeUint(result);
        assertEq(shares, amount, "Should receive 1:1 shares for Aave");
        assertEq(aToken.balanceOf(address(wallet)), amount, "Wallet should have aTokens");
        assertEq(feeTracker.agentVaultCostBasis(address(wallet), address(aToken)), amount, "Cost basis should be recorded");
        assertEq(feeTracker.agentVaultShares(address(wallet), address(aToken)), amount, "Shares should be recorded");
    }

    function test_Execute_DepositPercentage_UsesBalance() public {
        uint256 initialBalance = baseAsset.balanceOf(address(wallet));
        bytes memory result = wallet.executeAdapter(address(adapter), address(aToken), abi.encodeWithSelector(adapter.depositPercentage.selector, uint256(2500)));
        uint256 shares = _decodeUint(result);
    ...
    function test_Execute_DepositZeroAmount_Reverts() public {
        ...
        wallet.executeAdapter(address(adapter), address(aToken), abi.encodeWithSelector(adapter.deposit.selector, 0));
    }
    ...
    function test_Execute_Withdraw_Succeeds() public {
        wallet.executeAdapter(address(adapter), address(aToken), abi.encodeWithSelector(adapter.deposit.selector, 2_000e6));
        uint256 walletBalanceBefore = baseAsset.balanceOf(address(wallet));
        bytes memory result = wallet.executeAdapter(address(adapter), address(aToken), abi.encodeWithSelector(adapter.withdraw.selector, uint256(1_200e6)));
    ...
    function test_Execute_WithdrawZeroShares_Reverts() public {
        vm.expectRevert(abi.encodeWithSelector(AWKErrors.ZeroAmount.selector));
        wallet.executeAdapter(address(adapter), address(aToken), abi.encodeWithSelector(adapter.withdraw.selector, uint256(0)));
    }
    ...
    function test_VirtualShares_YieldAccrual_FeeCharged() public {
        ...
        (uint256 costBasisBefore, uint256 sharesBefore) = feeTracker.getAgentVaultPosition(address(wallet), address(aToken));
