...
contract CompoundV3AdapterTest is Test {
    ...
    function test_VirtualShares_YieldAccrual_FeeCharged() public {
        ...
        wallet.executeAdapter(address(adapter), address(comet), abi.encodeWithSelector(adapter.withdraw.selector, depositAmount));
    ...
    function test_VirtualShares_PartialWithdraw_WithYield() public {
        ...
        bytes memory result = wallet.executeAdapter(address(adapter), address(comet), abi.encodeWithSelector(adapter.withdraw.selector, withdrawVirtualShares));
        uint256 assetsReceived = _decodeUint(result);
