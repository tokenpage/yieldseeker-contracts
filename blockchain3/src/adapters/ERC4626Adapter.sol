...
contract ERC4626Adapter {
     ...
     *      Uses `self` immutable to check adapter matches (since address(this) is wallet in delegatecall)
     */
     ...
     *      1. Validate vault is registered for this adapter
     *      2. Approve vault to spend tokens (runs in wallet context)
     *      3. Call vault.deposit(amount, address(this))
     *      4. Vault transfers tokens from wallet, mints shares to wallet
     */
     ...
     * @dev Via delegatecall (address(this) = agentWallet):
     *      1. Validate vault is registered for this adapter
     *      2. Call vault.redeem(shares, address(this), address(this))
     *      3. Vault burns shares from wallet, sends assets to wallet
     */
